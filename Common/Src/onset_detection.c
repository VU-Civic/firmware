...
#define SPECTRAL_SPIKINESS_THRESHOLD           32.0f
...
#define XCORR_PHAT_MIN_HZ                      800
#define XCORR_PHAT_MAX_HZ                      8000
#define XCORR_PHAT_MIN_BIN                     (XCORR_PHAT_MIN_HZ * XCORR_FFT_SIZE / AUDIO_SAMPLE_RATE_HZ)
#define XCORR_PHAT_MAX_BIN                     (XCORR_PHAT_MAX_HZ * XCORR_FFT_SIZE / AUDIO_SAMPLE_RATE_HZ)
#define GCC_MIN_COHERENCE                      0.056f
...
#define REFINE_PEAK_FRACTION                   0.0945f
#define REFINE_NOISE_MULT                      7.0f
...
static int16_t previous_audio[AUDIO_NUM_CHANNELS][AUDIO_PACKET_NUM_SAMPLES];
static int16_t refinement_buf[REFINE_CTX_LEN], pending_window[FFT_WINDOW_SIZE];
...
static float gcc_buf[XCORR_FFT_SIZE], corr[AUDIO_NUM_CHANNELS - 1][XCORR_FFT_SIZE];
...
static int64_t refine_onset_using_ste(const int16_t *buff, int32_t nearby_offset, uint8_t nearby_offset_valid)
{
   ...
   // Threshold based on the true peak; for high-SNR shots the peak-fraction term dominates,
   ...
   // Pass 2: Backward scan from the coarse onset location toward the noise region
   int32_t onset_pos = -1;
...
static double calculate_interchannel_delays(int64_t onset_samp_abs, int64_t current_start, const int16_t (*audio_samples)[AUDIO_BUFFER_SAMPLES_PER_CHANNEL], double inter_channel_delays[AUDIO_NUM_CHANNELS - 1], int selected_ch, float *gcc_coherence)
{
   ...
   *gcc_coherence = best_score / (float)(AUDIO_NUM_CHANNELS - 1);
...
static float compute_spectral_spikiness(void)
{
   ...
   float peak, mean;
   arm_max_no_idx_f32(magnitudes + FFT_MIN_BIN, NUM_FFT_BINS + 1, &peak);
