...
#define FFT_MIN_BIN                            (SPECTROGRAM_MIN_FREQUENCY_HZ * FFT_FILTER_SIZE / AUDIO_PACKET_SAMPLE_RATE)
#define FFT_MAX_BIN                            (2 + (SPECTROGRAM_MAX_FREQUENCY_HZ * FFT_FILTER_SIZE / AUDIO_PACKET_SAMPLE_RATE))
...
#define FFT_FINE_MIN_BIN                       (SPECTROGRAM_MIN_FREQUENCY_HZ * FFT_FINE_FILTER_SIZE / AUDIO_PACKET_SAMPLE_RATE)
#define FFT_FINE_MAX_BIN                       (2 + (SPECTROGRAM_MAX_FREQUENCY_HZ * FFT_FINE_FILTER_SIZE / AUDIO_PACKET_SAMPLE_RATE))
...
#define BROADBAND_BAND_BINS                    (BROADBAND_ACTIVATION_BIN_SIZE_HZ * FFT_FILTER_SIZE / AUDIO_PACKET_SAMPLE_RATE)
#define BROADBAND_NOISE_BANDS                  (NUM_FFT_BINS / BROADBAND_BAND_BINS)

#define MIN_TIME_BETWEEN_ONSETS_MS             200
#define MIN_SAMPLES_BETWEEN_ONSETS             (MIN_TIME_BETWEEN_ONSETS_MS * AUDIO_PACKET_SAMPLE_RATE / 1000)
...
static float broadband_noise_floor[BROADBAND_NOISE_BANDS], magnitudes[NUM_FFT_BINS], prev_magnitudes[NUM_FFT_BINS];
...
static int32_t samples_since_prev_onset, packet_idx;
...
static float compute_flux_coarse(const int16_t *audio)
{
   ...
   arm_cmplx_mag_squared_f32(fft_buf + (2 * FFT_MIN_BIN), magnitudes, NUM_FFT_BINS);
   ...
   arm_copy_f32(magnitudes, prev_magnitudes, NUM_FFT_BINS);
   return flux;
}
...
static float broadband_activation(void)
{
   ...
   for (int b = 0; b < BROADBAND_NOISE_BANDS; ++b)
   {
      ...
      float band_energy;
      const int lo = b * BROADBAND_BAND_BINS;
...
static int check_onset_conditions(float flux, float adaptive_threshold)
{
   ...
   // Gate 3: Enforce minimum refractory period between onsets
   if (samples_since_prev_onset < MIN_SAMPLES_BETWEEN_ONSETS)
      return 0;
   return 1;
}

static int32_t onset_detector_refine(const int16_t *window, int32_t starting_idx)
{
   // Scan fine-grained FFT windows within the coarse detection frame to pinpoint onset
   int32_t onset_idx = starting_idx, onset_offset = 0;
...
static void process_time_step(const int16_t *window, int32_t audio_sample_idx, onset_details_t *onsets)
{
   ...
   // Check if this time step meets all conditions for an onset and, if so, refine its location and save it
   if (check_onset_conditions(flux, adaptive_threshold) && (onsets->num_onsets < MAX_NUM_ONSETS_PER_CLIP))
      onsets->indices[onsets->num_onsets++] = onset_detector_refine(window, audio_sample_idx);
   prev_flux = flux;
}
...
void onset_detection_init(void)
{
   ...
   arm_fill_f32(0.0f, magnitudes, NUM_FFT_BINS);
   arm_fill_f32(0.0f, prev_magnitudes, NUM_FFT_BINS);
   ...
   packet_idx = flux_warmup_count = 0;
...
onset_details_t onset_detection_invoke(const int16_t (*audio_samples)[AUDIO_BUFFER_SAMPLES_PER_CHANNEL], channel_alarms_t channel_alarms)
{
   ...
   onset_details_t onsets = { .num_onsets = 0 };
   for (int m = 0; m < MISSING_WINDOWS_PER_PACKET; ++m)
   {
      ...
      // Process the pending buffer as a complete window and save any detected onsets with their absolute audio sample indices
      const int32_t audio_sample_idx = ((packet_idx - 1) * AUDIO_PACKET_NUM_SAMPLES) + ((AVAILABLE_WINDOWS_PER_PACKET + m) * FFT_STEP_SIZE);
      process_time_step(pending_buf, audio_sample_idx, &onsets);
   ...
   {
      const int32_t audio_sample_idx = (packet_idx * AUDIO_PACKET_NUM_SAMPLES) + (t * FFT_STEP_SIZE);
      process_time_step(audio_packet + (t * FFT_STEP_SIZE), audio_sample_idx, &onsets);
   }
