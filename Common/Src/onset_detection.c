...
// Onset Detector Type Definitions -------------------------------------------------------------------------------------

struct onset_detection_state_t
{
   double previous_timestamp;
   int64_t previous_raw_onset_sample, previous_start_sample;
   arm_rfft_fast_instance_f32 fft_coarse, fft_fine, fft_xcorr;
   int16_t previous_audio[AUDIO_NUM_CHANNELS][AUDIO_PACKET_NUM_SAMPLES];
   int16_t refinement_buf[REFINE_CTX_LEN], pending_window[FFT_WINDOW_SIZE];
   float fft_buf[FFT_FILTER_SIZE], hanning_window[FFT_WINDOW_SIZE], hanning_window_fine[FFT_FINE_WINDOW_SIZE];
   float broadband_noise_floor[BROADBAND_NOISE_BANDS], magnitudes[FFT_FILTER_SIZE / 2], prev_magnitudes[FFT_FILTER_SIZE / 2];
   float magnitudes_fine[NUM_FFT_FINE_BINS], prev_magnitudes_fine[NUM_FFT_FINE_BINS], gcc_ch0_fft[XCORR_FFT_SIZE];
   float gcc_buf[XCORR_FFT_SIZE], corr[AUDIO_NUM_CHANNELS - 1][XCORR_FFT_SIZE];
   float flux_ema_mean, flux_ema_var, prev_flux;
   int32_t samples_since_prev_onset;
   uint32_t flux_warmup_count;
 #ifdef TELEMETRY_ENABLE_METRICS
   volatile uint32_t onset_invoke_count, onset_detect_count;
   volatile float onset_ms_last, onset_ms_max;
 #endif
};
...
static onset_detection_state_t detection_state;
...
static void build_refinement_buffer(onset_detection_state_t* state, int16_t *buff, int64_t onset_timestamp, int64_t current_start, const int16_t (*audio_samples)[AUDIO_BUFFER_SAMPLES_PER_CHANNEL], int selected_ch)
{
   // Compute the window boundaries of the previous and current packets relative to the detected onset
   const int32_t ctx_start_rel = (int32_t)(onset_timestamp - current_start) - REFINE_ONSET_IN_CTX, previous_offset = (int32_t)(state->previous_start_sample - current_start);
   ...
   if (previous_lo < previous_hi)
      arm_copy_q15(state->previous_audio[selected_ch] + (previous_lo + ctx_start_rel - previous_offset), buff + previous_lo, (uint32_t)(previous_hi - previous_lo));
...
static void fill_xcorr_window_q15(onset_detection_state_t* state, int16_t *out, int ch, int64_t onset_samp_abs, int64_t current_start, const int16_t (*audio_samples)[AUDIO_BUFFER_SAMPLES_PER_CHANNEL])
{
   const int32_t win_start_rel = (int32_t)(onset_samp_abs - (XCORR_WINDOW_SIZE / 2) - current_start), previous_offset = (int32_t)(state->previous_start_sample - current_start);
   ...
   if (previous_lo < previous_hi)
      arm_copy_q15(state->previous_audio[ch] + (previous_lo + win_start_rel - previous_offset), out + previous_lo, (uint32_t)(previous_hi - previous_lo));
   if (cur_lo < cur_hi)
      arm_copy_q15(audio_samples[ch] + (cur_lo + win_start_rel), out + cur_lo, (uint32_t)(cur_hi - cur_lo));
}

static void fill_xcorr_channel(onset_detection_state_t* state, float *out, int ch, int64_t onset_samp_abs, int64_t current_start, const int16_t (*audio_samples)[AUDIO_BUFFER_SAMPLES_PER_CHANNEL])
{
   int16_t xcorr_win[XCORR_WINDOW_SIZE];
   fill_xcorr_window_q15(state, xcorr_win, ch, onset_samp_abs, current_start, audio_samples);
   arm_q15_to_float(xcorr_win, out, XCORR_WINDOW_SIZE);
}

static void gcc_phat_correlate(onset_detection_state_t* state, float *out, int ch, int64_t onset_samp_abs, int64_t current_start, const int16_t (*audio_samples)[AUDIO_BUFFER_SAMPLES_PER_CHANNEL])
{
   // Create a window and take the FFT of the indicated channel
   fill_xcorr_channel(state, out, ch, onset_samp_abs, current_start, audio_samples);
   arm_fill_f32(0.0f, out + XCORR_WINDOW_SIZE, XCORR_FFT_SIZE - XCORR_WINDOW_SIZE);
   arm_rfft_fast_f32(&state->fft_xcorr, out, out, 0);

   // Compute the cross-spectrum gcc_ch0_fft * conj(out) for in-band bins into gcc_buf
   arm_copy_f32(state->gcc_ch0_fft, state->gcc_buf, XCORR_FFT_SIZE);
   state->gcc_buf[0] = state->gcc_buf[1] = 0.0f;
   for (int k = 1; k < XCORR_FFT_SIZE / 2; ++k)
   {
      if ((k < XCORR_PHAT_MIN_BIN) || (k > XCORR_PHAT_MAX_BIN))
      {
         state->gcc_buf[2*k] = state->gcc_buf[2*k + 1] = 0.0f;
         continue;
      }
      const float ar = state->gcc_buf[2*k], ai = state->gcc_buf[2*k + 1];
      const float br = out[2*k], bi = out[2*k + 1];
      state->gcc_buf[2*k] = (ar * br) + (ai * bi);
      state->gcc_buf[2*k + 1] = (ai * br) - (ar * bi);
   }

   // Batch-compute the FFT magnitudes, then normalize gcc_buf
   arm_cmplx_mag_f32(state->gcc_buf + 2, out, XCORR_FFT_SIZE / 2 - 1);
   for (int k = 1; k < XCORR_FFT_SIZE / 2; ++k)
   {
      ...
      if (mag > 1e-12f)
      {
         ...
         state->gcc_buf[2*k] *= inv_mag;
         state->gcc_buf[2*k + 1] *= inv_mag;
      }
      else
         state->gcc_buf[2*k] = state->gcc_buf[2*k + 1] = 0.0f;
   }
   ...
   arm_rfft_fast_f32(&state->fft_xcorr, state->gcc_buf, out, 1);
}

static double calculate_interchannel_delays(onset_detection_state_t* state, int64_t onset_samp_abs, int64_t current_start, const int16_t (*audio_samples)[AUDIO_BUFFER_SAMPLES_PER_CHANNEL], double inter_channel_delays[AUDIO_NUM_CHANNELS-1], int selected_ch)
{
   // Compute and cache the ch0 FFT once
   fill_xcorr_channel(state, state->gcc_ch0_fft, 0, onset_samp_abs, current_start, audio_samples);
   arm_fill_f32(0.0f, state->gcc_ch0_fft + XCORR_WINDOW_SIZE, XCORR_FFT_SIZE - XCORR_WINDOW_SIZE);
   arm_rfft_fast_f32(&state->fft_xcorr, state->gcc_ch0_fft, state->gcc_ch0_fft, 0);

   // Compute the GCC-PHAT correlation function for each channel pair
   for (int c = 1; c < AUDIO_NUM_CHANNELS; ++c)
      gcc_phat_correlate(state, state->corr[c - 1], c, onset_samp_abs, current_start, audio_samples);
   ...
   for (int d1 = d1_min; d1 <= XCORR_MAX_DELAY_SAMP; ++d1)
   {
      ...
      const float s1 = state->corr[0][i1];
      for (int d2 = -XCORR_MAX_DELAY_SAMP; d2 <= XCORR_MAX_DELAY_SAMP; ++d2)
      {
         ...
         const float s12 = s1 + state->corr[1][i2];
         for (int dd = -1; dd <= 1; ++dd)
         {
            const int d3 = d1 + d2 + dd;
            const int i3 = (d3 >= 0) ? d3 : (XCORR_FFT_SIZE + d3);
            const float score = s12 + state->corr[2][i3];
            if (score > best_score)
            {
               best_score = score;
   ...
   q63_t power_raw;
   int16_t xcorr_win[XCORR_WINDOW_SIZE];
   fill_xcorr_window_q15(state, xcorr_win, selected_ch, onset_samp_abs, current_start, audio_samples);
   arm_power_q15(xcorr_win, XCORR_WINDOW_SIZE, &power_raw);
   return sqrt((double)power_raw / (XCORR_WINDOW_SIZE * 1073741824.0));
}

static float compute_flux_coarse(onset_detection_state_t* state, const int16_t *audio)
{
   // Convert Q15 audio to float in a zero-padded FFT buffer
   arm_q15_to_float(audio, state->fft_buf, FFT_WINDOW_SIZE);
   arm_fill_f32(0.0f, state->fft_buf + FFT_WINDOW_SIZE, FFT_FILTER_SIZE - FFT_WINDOW_SIZE);

   // Apply Hanning window to the unpadded signal region and compute in-place forward real FFT
   arm_mult_f32(state->fft_buf, state->hanning_window, state->fft_buf, FFT_WINDOW_SIZE);
   arm_rfft_fast_f32(&state->fft_coarse, state->fft_buf, state->fft_buf, 0);

   // Compute the power spectrum (magnitude squared) for all frequency bins
   arm_cmplx_mag_squared_f32(state->fft_buf, state->magnitudes, FFT_FILTER_SIZE / 2);

   // Positive spectral flux: sum of per-bin energy increases only
   float flux = 0.0f;
   for (int i = FFT_MIN_BIN; i <= FFT_MAX_BIN; ++i)
   {
      const float delta = state->magnitudes[i] - state->prev_magnitudes[i];
      if (delta > 0.0f)
         flux += delta;
   }

   // Store the magnitude spectrum for the next iteration
   arm_copy_f32(state->magnitudes, state->prev_magnitudes, FFT_FILTER_SIZE / 2);
   return flux;
}

static float compute_flux_fine(onset_detection_state_t* state, const int16_t *audio)
{
   // Convert Q15 audio to float, apply Hanning window, and compute in-place forward real FFT
   arm_q15_to_float(audio, state->fft_buf, FFT_FINE_WINDOW_SIZE);
   arm_mult_f32(state->fft_buf, state->hanning_window_fine, state->fft_buf, FFT_FINE_WINDOW_SIZE);
   arm_rfft_fast_f32(&state->fft_fine, state->fft_buf, state->fft_buf, 0);

   // Power spectrum (magnitude squared) for bins of interest
   arm_cmplx_mag_squared_f32(state->fft_buf + (2 * FFT_FINE_MIN_BIN), state->magnitudes_fine, NUM_FFT_FINE_BINS);

   // Positive spectral flux: sum of per-bin energy increases only
   float flux = 0.0f;
   for (int i = 0; i < NUM_FFT_FINE_BINS; i++)
   {
      const float delta = state->magnitudes_fine[i] - state->prev_magnitudes_fine[i];
      if (delta > 0.0f)
         flux += delta;
   }

   // Store the fine magnitude spectrum for the next iteration
   arm_copy_f32(state->magnitudes_fine, state->prev_magnitudes_fine, NUM_FFT_FINE_BINS);
   return flux;
}

static float update_flux_statistics(onset_detection_state_t* state, float flux)
{
   // Welford online mean/variance during warmup: accumulate simple running averages
   if (state->flux_warmup_count < FLUX_WARMUP_FRAMES)
   {
      const float n = ++state->flux_warmup_count;
      const float mean_new = state->flux_ema_mean + ((flux - state->flux_ema_mean) / n);
      if (state->flux_warmup_count > 1)
         state->flux_ema_var += (((flux - mean_new) * (flux - mean_new)) - state->flux_ema_var) / n;
      state->flux_ema_mean = mean_new;
      return FLUX_MIN_THRESHOLD;
   }

   // Exponential moving average update post-warmup
   const float delta = flux - state->flux_ema_mean;
   state->flux_ema_mean += FLUX_EMA_ALPHA * delta;
   state->flux_ema_var = (1.0f - FLUX_EMA_ALPHA) * (state->flux_ema_var + (FLUX_EMA_ALPHA * delta * delta));

   // Adaptive threshold: mean + (k * stddev), floored at the absolute minimum
   const float stddev = sqrtf(state->flux_ema_var > 0.0f ? state->flux_ema_var : 0.0f);
   const float threshold = state->flux_ema_mean + (FLUX_THRESHOLD_NUM_SIGMAS * stddev);
   return (threshold > FLUX_MIN_THRESHOLD) ? threshold : FLUX_MIN_THRESHOLD;
}

static float broadband_activation(onset_detection_state_t* state)
{
   ...
   for (int b = 0; b < BROADBAND_NOISE_BANDS; ++b)
   {
      ...
      arm_mean_f32(state->magnitudes + lo, BROADBAND_BAND_BINS, &band_energy);

      // Slow EMA noise floor: initialise from first non-zero frame
      if (state->broadband_noise_floor[b] < 1e-12f)
         state->broadband_noise_floor[b] = band_energy;
      else
         state->broadband_noise_floor[b] += BROADBAND_NOISE_EMA_ALPHA * (band_energy - state->broadband_noise_floor[b]);

      // Band is active if energy exceeds noise floor by the required margin
      if (band_energy > (state->broadband_noise_floor[b] * BROADBAND_NOISE_RISE_LIN))
         active_bands += 1.0f;
   }
   return active_bands / BROADBAND_NOISE_BANDS;
}

static float compute_energy_band_ratio(onset_detection_state_t* state)
{
   // Compute the ratio of total energy in the high band to the low band
   float total_energy_high, total_energy_low;
   arm_accumulate_f32(state->magnitudes + ENERGY_BAND_RATIO_LOW_BIN, ENERGY_BAND_RATIO_MID_BIN - ENERGY_BAND_RATIO_LOW_BIN + 1, &total_energy_low);
   arm_accumulate_f32(state->magnitudes + ENERGY_BAND_RATIO_MID_BIN, ENERGY_BAND_RATIO_HIGH_BIN - ENERGY_BAND_RATIO_MID_BIN, &total_energy_high);
   return total_energy_high / (total_energy_low + 1e-12);
}

static int check_onset_conditions(onset_detection_state_t* state, float flux, float adaptive_threshold)
{
   // Gate 1: Enough frequency bands must be activated above their noise floor
   if (broadband_activation(state) < BROADBAND_ACTIVATION_MIN_THRESHOLD)
      return 0;

   // Gate 2: Flux must exceed the adaptive threshold and be at a local peak
   if ((flux <= adaptive_threshold) || (flux < state->prev_flux))
      return 0;

   // Gate 3: Energy distribution must have enough low-frequency content
   if (compute_energy_band_ratio(state) > ENERGY_BAND_RATIO_THRESHOLD)
      return 0;

   // Gate 4: Enforce minimum refractory period between onsets
   if (state->samples_since_prev_onset < MIN_SAMPLES_BETWEEN_ONSETS)
      return 0;
   return 1;
}

static int32_t onset_detector_refine(onset_detection_state_t* state, const int16_t *window)
{
   // Scan fine-grained FFT windows within the coarse detection frame to pinpoint the onset
   int32_t onset_starting_sample = 0;
   float threshold = 2.0f * compute_flux_fine(state, window);
   for (int32_t off = FFT_FINE_STEP_SIZE; off <= (FFT_WINDOW_SIZE - FFT_FINE_WINDOW_SIZE); off += FFT_FINE_STEP_SIZE)
   {
      const float flux = compute_flux_fine(state, window + off);
      if (flux > threshold)
      {
         threshold = 2.0f * flux;
   ...
   state->samples_since_prev_onset = -onset_starting_sample;
   return onset_starting_sample + FFT_FINE_WINDOW_SIZE - FFT_FINE_STEP_SIZE;
}

static int32_t process_time_step(onset_detection_state_t* state, const int16_t *window)
{
   // Calculate spectral flux on the current window and update the adaptive threshold
   const float flux = compute_flux_coarse(state, window);
   const float adaptive_threshold = update_flux_statistics(state, flux);
   state->samples_since_prev_onset += FFT_STEP_SIZE;

   // Check if this time step meets all conditions for an onset
   int32_t onset_starting_sample = -1;
   if (check_onset_conditions(state, flux, adaptive_threshold))
      onset_starting_sample = onset_detector_refine(state, window);
...
onset_detection_state_t* onset_detection_create(void) { return &detection_state; }
...
void onset_detection_init(onset_detection_state_t *state)
{
   ...
   arm_rfft_init(&state->fft_coarse);
   arm_rfft_fine_init(&state->fft_fine);
   arm_rfft_xcorr_init(&state->fft_xcorr);
   arm_hanning_f32(state->hanning_window, FFT_WINDOW_SIZE);
   arm_hanning_f32(state->hanning_window_fine, FFT_FINE_WINDOW_SIZE);

   // Zero the detection state
   arm_fill_f32(0.0f, state->magnitudes, FFT_FILTER_SIZE / 2);
   arm_fill_f32(0.0f, state->prev_magnitudes, FFT_FILTER_SIZE / 2);
   arm_fill_f32(0.0f, state->magnitudes_fine, NUM_FFT_FINE_BINS);
   arm_fill_f32(0.0f, state->prev_magnitudes_fine, NUM_FFT_FINE_BINS);
   arm_fill_f32(0.0f, state->broadband_noise_floor, BROADBAND_NOISE_BANDS);
   arm_fill_q15(0, state->refinement_buf, REFINE_CTX_LEN);
   memset(state->previous_audio, 0, sizeof(state->previous_audio));
   state->flux_ema_mean = state->flux_ema_var = state->prev_flux = 0.0f;
   state->flux_warmup_count = 0;
   state->samples_since_prev_onset = 10000000;
   state->previous_timestamp = 0.0;
   state->previous_start_sample = 0;
   state->previous_raw_onset_sample = -1;

   // Clear all telemetry metrics
#ifdef TELEMETRY_ENABLE_METRICS
   state->onset_invoke_count = state->onset_detect_count = 0;
   state->onset_ms_last = state->onset_ms_max = 0.0f;
#endif
}
...
void onset_detection_invoke(onset_detection_state_t *state, double packet_timestamp, const int16_t (*audio_samples)[AUDIO_BUFFER_SAMPLES_PER_CHANNEL], volatile data_packet_t* const packet)
{
   ...
   for (int m = 0; m < MISSING_WINDOWS_PER_PACKET; ++m)
   {
      ...
      arm_copy_q15(&state->previous_audio[selected_ch][AUDIO_PACKET_NUM_SAMPLES - left_len], state->pending_window, left_len);
      arm_copy_q15(audio_packet, state->pending_window + left_len, FFT_WINDOW_SIZE - left_len);
      const int32_t onset_sample = process_time_step(state, state->pending_window);
      if (onset_sample >= 0)
   ...
   {
      const int32_t onset_sample = process_time_step(state, audio_packet + (t * FFT_STEP_SIZE));
      if (onset_sample >= 0)
   ...
   if (onset_timestamp > 0.0)
   {
      ...
      build_refinement_buffer(state, state->refinement_buf, onset_sample_raw, current_start, audio_samples, selected_ch);
      const int64_t onset_delta = refine_onset_using_ste(state->refinement_buf, (int32_t)(state->previous_raw_onset_sample - onset_sample_raw), state->previous_raw_onset_sample >= 0);
      onset_timestamp += (double)onset_delta / AUDIO_SAMPLE_RATE_HZ;
      state->previous_raw_onset_sample = onset_sample_raw + onset_delta;
      ...
      const double onset_magnitude = calculate_interchannel_delays(state, state->previous_raw_onset_sample, current_start, audio_samples, inter_channel_delays, selected_ch);
   ...
   state->previous_timestamp = packet_timestamp;
   state->previous_start_sample = current_start;
   for (int c = 0; c < AUDIO_NUM_CHANNELS; ++c)
      arm_copy_q15(audio_samples[c], state->previous_audio[c], AUDIO_PACKET_NUM_SAMPLES);

   // Update the most recent telemetry metrics
#ifdef TELEMETRY_ENABLE_METRICS
   state->onset_ms_last = 1000.0f * ((float)(READ_REG(DWT->CYCCNT) - cycle_start) / SystemCoreClock);
   ++state->onset_invoke_count;
   if (onset_timestamp > 0.0)
      ++state->onset_detect_count;
   if (state->onset_ms_last > state->onset_ms_max)
      state->onset_ms_max = state->onset_ms_last;
#endif
}
